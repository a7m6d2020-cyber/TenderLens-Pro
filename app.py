...
الهدف ليس التلخيص، بل استخراج سجل متطلبات موثق يمكن دمجه لاحقاً في تقرير العرض الفني.

قواعد إلزامية:
1. اعتمد فقط على نص الملف المقدم لك. لا تستنتج جهة مالكة أو موقعاً أو نطاقاً من خارج النص.
2. إذا ظهرت أسماء جهات أو مواقع متعددة داخل الملف، لا تحسمها؛ ضعها تحت [تعارض/يحتاج تحقق].
3. لا تستخدم عبارات عامة مثل "الالتزام بالمواصفات" وحدها؛ اذكر المتطلب أو المعيار أو الإجراء كما ظهر في النص قدر الإمكان.
4. استخرج الأرقام حرفياً: مدد، مبالغ، نسب، كميات، ضمانات، غرامات، تواريخ، مدد صلاحية، مدد تنفيذ، فترات ضمان.
5. إذا لم تجد معلومة، اكتب: [غير متوفر في هذا الملف].
6. أي بند كمية صفر يظهر في جدول الكميات يُوضع في قسم مستقل [بنود كمية صفر تحتاج تحقق] ولا يُدرج ضمن النطاق التنفيذي المعتمد.
7. لا تختصر المتطلبات الحرجة. أعطِ الأولوية للمتطلبات التي تؤثر على قبول العرض أو تقييمه أو محتوى العرض الفني.

أعد المخرج بالهيكل التالي:

# 1. تعريف الملف ومصداقية الاستخراج
- اسم الملف كما ورد.
- نوع المعلومات المستخرجة منه.
- مستوى وضوح النص: عالٍ / متوسط / ضعيف.
- أي ملاحظات قراءة أو نقص OCR إن ظهر.

# 2. حقائق المشروع الظاهرة في هذا الملف
جدول بالأعمدة: الحقل، القيمة كما وردت، درجة الثقة، المصدر داخل الملف إن توفر.
الحقول تشمل: اسم المشروع، الجهة المالكة، الجهة المشرفة، الموقع، رقم المنافسة/العقد، مدة المشروع، نوع العقد، موعد التقديم، نطاق عام.

# 3. سجل متطلبات الجهة المالكة من مقدم العرض
جدول تفصيلي بالأعمدة:
- رقم
- نص/مضمون المتطلب المطلوب من مقدم العرض
- نوع المتطلب: فني / تقديمي / إداري / جودة / سلامة / بيئة / موارد / برنامج زمني / وثائق / مالي-تعاقدي
- درجة الأهمية: حرج / رئيسي / داعم
- الدليل أو العبارة الدالة من النص
- أثر المتطلب على عرض شركة الرواف
- الإجراء المطلوب من شركة الرواف

# 4. المتطلبات الفنية والكودية والاختبارات
استخرج المواصفات، الأكواد، المعايير، المواد، الاختبارات، متطلبات الاعتماد، العينات، طرق التنفيذ، وحدود القبول.

# 5. المنهجيات والخطط والوثائق المطلوبة في العرض الفني
اذكر كل خطة أو Method Statement أو برنامج أو جدول أو سجل أو مستند مطلوب تقديمه أو يلزم إعداده لإثبات الامتثال.

# 6. الموارد والكوادر والمعدات
استخرج أي اشتراط على الكوادر، الخبرات، الشهادات، الهيكل التنظيمي، المعدات، العمالة، الموردين، المقاولين الباطن، أو المحتوى المحلي.

# 7. الجودة والسلامة والبيئة والتصاريح
استخرج متطلبات الجودة، الفحوصات، السلامة، الصحة المهنية، البيئة، التصاريح، الاعتمادات، التنسيق مع الجهات، ونقاط التفتيش.

# 8. البرنامج الزمني والمواعيد والضمانات والغرامات
استخرج جميع الالتزامات الزمنية والمواعيد والمدد والغرامات والضمانات وفترات الصلاحية.

# 9. جدول الكميات والأرقام المؤثرة
استخرج البنود والكميات والأرقام المؤثرة على النطاق الفني. ميّز بنود كمية صفر إن وجدت.

# 10. التعارضات والفجوات والاستفسارات
جدول بالأعمدة: الرقم، الفجوة/التعارض/السؤال، سبب الأهمية، النص/المصدر، الإجراء المقترح.
"""

TENDER_SYNTHESIS_PROMPT = """أنت مهندس فني أول لإعداد العروض الفنية وخبير تحليل وثائق مناقصات في قطاع المقاولات.

مهمتك دمج مخرجات تحليل الملفات في تقرير نهائي لصالح مقدم العرض: شركة الرواف.
هذا التقرير يجب أن يكون تقرير عمل فعلي لفريق العروض الفنية، وليس ملخصاً عاماً.

قواعد إلزامية:
1. اعتمد فقط على مخرجات التحليل المزودة لك. لا تضف أي معلومة من خارجها.
2. إذا وُجد تعارض بين الجهة المالكة أو الموقع أو اسم المشروع أو المدة أو النطاق، لا تحسم التعارض؛ ضعه في قسم [تعارضات تحتاج حسم].
3. لا تخلط بين جهات أو مواقع مختلفة. إذا ظهر مثلاً اسم جهة وموقع لا يبدوان متسقين، اذكر ذلك بوضوح كخطر خلط وثائقي.
4. اجعل محور التقرير: "ماذا تطلب الجهة المالكة من مقدم العرض/شركة الرواف تحديداً؟".
5. لا تقبل صياغات عامة مثل "تقديم جميع الوثائق" دون تفصيل أنواع الوثائق إذا ظهرت.
6. ضع [غير متوفر في الوثائق المرفوعة] لأي حقل غير مثبت.
7. استخرج الأرقام والنسب والمدد والكميات والضمانات والغرامات كما هي.
8. لا تدرج البنود ذات كمية صفر ضمن نطاق التنفيذ؛ ضعها تحت [بنود كمية صفر تحتاج تحقق].

هيكل التقرير النهائي المطلوب:

# 📋 الملخص التنفيذي للمنافسة
- فقرة دقيقة عن طبيعة المشروع بناءً على الوثائق فقط.
- أهم 8 إلى 12 متطلباً حرجاً من الجهة المالكة لمقدم العرض.
- أهم التعارضات أو النواقص التي قد تؤثر على قبول العرض.
- تنبيه صريح إذا كانت هوية المشروع/الجهة/الموقع غير متسقة بين الوثائق.

# 1. بطاقة حقائق المشروع
جدول بالأعمدة: الحقل، القيمة المستخلصة، المصدر/الملف، حالة الثقة: مؤكد / يحتاج تحقق / تعارض / غير متوفر.
الحقول الإلزامية: اسم المشروع، رقم المنافسة، الجهة المالكة، الجهة المشرفة، الموقع، مدة التنفيذ، نوع العقد، موعد التقديم، نطاق عام، مستندات مرجعية.

# 2. سجل وثائق المناقصة المحللة
جدول بالأعمدة: رقم، اسم الملف، نوع المعلومات التي وفرها، أهم المتطلبات المستخرجة منه، ملاحظات جودة القراءة.

# 3. نطاق العمل المستخلص والمعتمد مبدئياً
- اعرض النطاق كحزم تنفيذية.
- لكل حزمة: الأعمال المشمولة، المصدر، القيود، المخرجات المتوقعة.
- لا تضف أي بند غير ظاهر في الوثائق.

# 4. مصفوفة متطلبات الجهة المالكة من مقدم العرض
جدول تفصيلي لا يقل عن 20 متطلباً متى سمحت الوثائق بذلك، بالأعمدة:
- رقم
- المتطلب المطلوب من مقدم العرض
- التصنيف: فني / تقديمي / إداري / جودة / سلامة / بيئة / زمني / موارد / وثائق / مالي-تعاقدي
- درجة الأهمية: حرج / رئيسي / داعم
- المصدر: اسم الملف
...
جدول/قوائم تفصيلية تشمل الأكواد، المواصفات، المواد، الاختبارات، الاعتمادات، العينات، طرق التنفيذ، حدود القبول، وأي متطلبات رقمية.

# 6. المنهجيات والخطط المطلوبة ضمن العرض الفني
جدول بالأعمدة: الخطة/المنهجية المطلوبة، سبب الحاجة، المصدر، محتوى مقترح يجب أن يتضمنه العرض الفني، الأولوية.
يشمل ذلك: خطة التنفيذ، خطة الجودة، خطة السلامة، خطة البيئة، خطة إدارة المخاطر، خطة الموارد، البرنامج الزمني، خطة إدارة المرور/التصاريح إن ظهرت أو لزم إثباتها من النطاق.

# 7. الموارد والكوادر والمعدات المطلوبة
استخرج أي اشتراطات مباشرة أو غير مباشرة على الكوادر والمعدات والعمالة والموردين والمقاولين الباطن والمحتوى المحلي. لا تخترع أعداداً غير مذكورة.

# 8. متطلبات الجودة والسلامة والبيئة والتصاريح
استخرج كل متطلبات الجودة والفحص والسلامة والصحة المهنية والبيئة والتنسيق والتصاريح والاعتمادات.

# 9. البرنامج الزمني والمواعيد والالتزامات الزمنية
جدول بالأعمدة: الالتزام الزمني، التاريخ/المدة، المصدر، أثره على العرض، الإجراء المطلوب.

# 10. جدول الكميات والأرقام المؤثرة
استخرج الأرقام والكميات والبنود المؤثرة على فهم النطاق الفني. ضع قسم مستقل لبنود كمية صفر إن وجدت.

# 11. الشروط المالية/التعاقدية المؤثرة على العرض الفني
استخرج شروط الضمانات، التأمينات، الغرامات، الدفع، الاعتمادات، المسؤوليات، وفترات الضمان التي تؤثر على تجهيز العرض الفني.

# 12. سجل التعارضات والفجوات ونقاط الاستفسار
جدول بالأعمدة: رقم، النوع: تعارض/فجوة/استفسار، الوصف، سبب الخطورة، الملفات ذات العلاقة، الإجراء المقترح قبل التقديم.

# 13. توصيات إعداد العرض الفني لشركة الرواف
قائمة تنفيذية مرتبة بالأولوية: ما يجب إدراجه، ما يجب التحقق منه، ما يجب تجنبه، وما يجب سؤاله للجهة المالكة.
"""

def _is_weak_tender_report(report: str) -> bool:
    """Quality gate: detect generic/weak tender reports and trigger staged extraction."""
    if not report or report.startswith("[AI Error"):
        return False
    wc = word_count(report)
    required_markers = [
        "مصفوفة متطلبات الجهة المالكة",
        "بطاقة حقائق المشروع",
        "سجل وثائق المناقصة",
        "الفجوات",
        "توصيات إعداد العرض الفني",
    ]
    missing_markers = sum(1 for m in required_markers if m not in report)
    generic_hits = sum(report.count(p) for p in [
        "الالتزام بالمعايير",
        "تقديم جميع الوثائق",
        "توفير الكوادر",
    ...
    # A serious tender analysis should be dense. Short/generic reports are automatically rejected.
    return wc < 2200 or missing_markers >= 2 or generic_hits >= 5


def analyze_tender_in_batches(client, tender_texts: dict) -> str:
    """
    Deep tender analysis engine.

    Preferred behavior:
    - One comprehensive focused analysis when the uploaded context fits a safe token budget.

    Quality/safety behavior:
    - If the one-pass result is weak/generic or the tender set is too large, switch to staged
      evidence extraction per file, then synthesize a rich owner-requirements report.

    This preserves the user's preference for one focused report while preventing shallow summaries
    and avoiding Request too large errors.
    """
    if not tender_texts:
    ...
    def _single_pass() -> str:
        ...
        user_msg = (
            ...
            "أصدر تقريراً واحداً مركزاً وغزيراً ودقيقاً. التقرير يجب أن يجيب تحديداً: ماذا تطلب الجهة المالكة من مقدم العرض؟\n"
            "لا تكتب عبارات عامة. لا تخلط بين مشاريع أو جهات أو مواقع. إذا تعارضت المعلومات، ضعها تحت [تعارض].\n"
            "استخرج المتطلبات الفنية والتقديمية والإدارية والزمنية والموارد والجودة والسلامة والوثائق والضمانات والأرقام.\n"
            "اربط كل متطلب باسم الملف قدر الإمكان.\n\n"
    ...
    def _staged_pass() -> str:
        ...
        for i, (name, txt) in enumerate(tender_texts.items(), start=1):
            ...
            file_prompt = (
                ...
                "لا تلخص الملف تلخيصاً عاماً. المطلوب استخراج سجل متطلبات موثق من هذا الملف فقط، "
                "مع كل الطلبات الفنية والتقديمية والإدارية والوثائق والموارد والجودة والسلامة والمواعيد والكميات والأرقام. "
                "إذا ظهر تعارض أو معلومة غير مؤكدة، اذكرها ولا تحسمها.\n\n"
            ...
            if summary.startswith("[AI Error"):
                ...
                # Preserve enough evidence from each file. Over-compression was the main cause of shallow reports.
                per_file_summaries.append(f"# {name}\n{truncate_to_token_budget(summary, 4_500, model=model)}")

            time.sleep(TENDER_ANALYSIS_SLEEP_SECONDS)

        merged_summaries = "\n\n".join(per_file_summaries)
        final_input = (
            "فيما يلي سجلات متطلبات مستخرجة من كل ملف على حدة. استخدمها لإصدار تقرير نهائي واحد لصالح شركة الرواف.\n"
            "يجب أن يكون التقرير غزيراً ودقيقاً، وأن يحتوي على كل ما تطلبه الجهة المالكة من مقدم العرض.\n"
            "لا تحذف المتطلبات بحجة الاختصار. لا تكتب عبارات عامة. لا تخلط بين جهات أو مواقع أو مشاريع.\n"
            "إذا كانت هوية المشروع أو الجهة المالكة أو الموقع غير متسقة بين الملفات، ضعها في قسم التعارضات.\n\n"
    ...
    if raw_total_tokens <= MAX_SINGLE_PASS_TENDER_TOKENS:
        result = _single_pass()
        if is_request_too_large_error(result):
            log.warning("Single-pass tender analysis exceeded request limits; switching to staged extraction.")
        elif _is_weak_tender_report(result):
            log.warning("Single-pass tender analysis quality gate failed; switching to staged extraction.")
        else:
            return result
