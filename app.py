...
TENDER_REPORT_MAX_TOKENS = 14000
API_TIMEOUT = 120.0
API_MAX_RETRIES = 3
...
MAX_INPUT_TOKENS_PER_FILE = 18_000
MAX_SYNTHESIS_INPUT_TOKENS = 70_000
...
MAX_SINGLE_PASS_TENDER_TOKENS = 28_000
...
SINGLE_FILE_TENDER_PROMPT = """أنت مهندس فني أول لإعداد العروض الفنية وخبير تحليل وثائق مناقصات.

مهمتك تحليل ملف واحد فقط من ملفات المناقصة لصالح مقدم العرض: شركة الرواف.
الهدف: استخراج سجل متطلبات تفصيلي وليس تلخيصاً.

قواعد إلزامية:
1. اعتمد فقط على نص الملف المقدم. لا تستنتج ولا تكمّل من خبرتك.
2. استخرج المتطلبات التي تطلبها الجهة من مقدم العرض صراحة أو التي تؤثر على محتوى العرض الفني.
3. اكتب الأرقام كما وردت: تواريخ، مدد، نسب، ضمانات، غرامات، كميات، حدود قبول، خبرات، نسخ، مبالغ.
4. لا تكتب "الالتزام بالمواصفات" وحدها؛ اذكر المواصفة/الكود/الفحص/المادة/الإجراء إن ظهر.
...
جدول تفصيلي. لا تقلل عدد الصفوف إذا كان الملف غنيًا. الأعمدة:
رقم | المتطلب المحدد كما يفهم من النص | التصنيف | درجة الأهمية | الدليل/النص الدال | أثره على عرض شركة الرواف | الإجراء المطلوب من شركة الرواف.

# 4. متطلبات التقديم والتأهيل والوثائق
استخرج كل ما يجب إرفاقه أو إثباته أو توقيعه أو تقديمه.

# 5. المتطلبات الفنية والكودية والاختبارات
استخرج المواصفات، الأكواد، المواد، الاختبارات، العينات، الاعتمادات، طرق التنفيذ، وحدود القبول.

# 6. المنهجيات والخطط المطلوبة
استخرج أي خطة، برنامج، بيان طريقة تنفيذ، سجل، جدول، إجراء، أو تقرير يجب تقديمه أو تضمينه في العرض.

# 7. الموارد والكوادر والمعدات والمحتوى المحلي
استخرج متطلبات الخبرات، العمالة، المعدات، الموردين، المقاولين الباطن، المحتوى المحلي، والتقارير المرتبطة.

# 8. الجودة والسلامة والبيئة والتصاريح والتنسيق
استخرج متطلبات الجودة، الفحوصات، السلامة، البيئة، التصاريح، التنسيق مع الجهات، وإدارة الحركة إن ظهرت.

# 9. البرنامج الزمني والضمانات والغرامات والشروط المؤثرة
استخرج المواعيد والمدد وفترات الصلاحية والضمانات والغرامات وشروط الدفع/التعاقد المؤثرة على العرض.

# 10. الكميات والأرقام المؤثرة
استخرج البنود والكميات والأرقام ذات الأثر الفني. افصل بنود كمية صفر.

# 11. التعارضات والفجوات والاستفسارات من هذا الملف
جدول: النوع | الوصف | سبب الأهمية | الإجراء المقترح.
"""

TENDER_SYNTHESIS_PROMPT = """أنت مهندس فني أول لإعداد العروض الفنية وخبير تحليل وثائق مناقصات في قطاع المقاولات.

مهمتك دمج سجلات تحليل الملفات في تقرير نهائي واحد لصالح مقدم العرض: شركة الرواف.
التقرير ليس ملخصاً؛ هو سجل عمل لفريق العروض الفنية لتجهيز عرض مطابق لمتطلبات الجهة المالكة.

قواعد إلزامية:
1. اعتمد فقط على سجلات الملفات المزودة لك.
2. إذا ظهر تعارض بين اسم المشروع أو الموقع أو الجهة أو المدة، لا تحسم؛ ضع [تعارض] مع الملفات ذات العلاقة.
3. محور التقرير: ماذا تطلب الجهة المالكة من مقدم العرض/شركة الرواف تحديداً؟
4. مصفوفة متطلبات الجهة المالكة يجب أن تكون غنية. استهدف 45 إلى 80 متطلباً إذا كانت السجلات تسمح.
5. لا تدمج عدة متطلبات في صف واحد. افصل كل وثيقة/خطة/ضمان/اختبار/اعتماد/مورد/التزام زمني في صف مستقل.
6. لا تكتب مصدرًا عامًا فقط إن توفر دليل مختصر؛ ضع اسم الملف والدليل الدال.
7. لا تخترع أرقاماً أو موارد. غير المثبت = [غير متوفر في الوثائق المرفوعة].
8. لا تجعل التوصيات بديلة عن المتطلبات؛ المتطلبات أولاً ثم التوصيات.

هيكل التقرير النهائي المطلوب:

# 📋 الملخص التنفيذي للمنافسة
- طبيعة المشروع وفق الوثائق فقط.
- درجة اتساق الوثائق.
- أهم 12 إلى 18 متطلباً حرجاً من الجهة المالكة.
- أهم التعارضات والفجوات المؤثرة.

# 1. بطاقة حقائق المشروع
جدول: الحقل | القيمة المستخلصة | المصدر/الملف | حالة الثقة.
الحقول: اسم المشروع، رقم المنافسة، الجهة المالكة، الجهة المشرفة، الموقع، مدة التنفيذ، نوع العقد، موعد التقديم، نطاق عام، مستندات مرجعية.

# 2. سجل وثائق المناقصة المحللة
جدول: رقم | اسم الملف | نوع الوثيقة | أهم المعلومات والمتطلبات المستخرجة | ملاحظات القراءة.

# 3. نطاق العمل المستخلص والمعتمد مبدئياً
اعرض الحزم التنفيذية كما ظهرت فقط. لكل حزمة: الأعمال، الكميات المؤثرة إن ظهرت، المصدر، القيود، المخرجات. افصل البنود ذات كمية صفر.

# 4. مصفوفة متطلبات الجهة المالكة من مقدم العرض
جدول تفصيلي بالأعمدة:
رقم | المتطلب المطلوب من مقدم العرض | التصنيف | درجة الأهمية | المصدر/الدليل | أثره على العرض الفني | الإجراء المطلوب من شركة الرواف.
يجب أن يكون هذا القسم هو الأكبر والأغزر في التقرير.

# 5. المتطلبات الفنية والكودية والاختبارات
جدول: المجال | المتطلب الفني/الكودي/الاختبار | الرقم/المعيار/القيمة إن وجدت | المصدر | أثره على المنهجية الفنية.

# 6. المنهجيات والخطط والوثائق المطلوبة ضمن العرض الفني
جدول: الخطة/الوثيقة | هل مطلوبة صراحة أم مستنتجة لإثبات الامتثال | المصدر | محتوى يجب أن يتضمنه عرض شركة الرواف | الأولوية.

# 7. الموارد والكوادر والمعدات والموردون والمقاولون الباطن
جدول: المورد/الكادر/المعدة/المورد | المتطلب | هل العدد/الخبرة مثبتة؟ | المصدر | إجراء الرواف.

# 8. الجودة والسلامة والبيئة والتصاريح والتنسيق
جدول: المجال | المتطلب | التقرير/السجل/الفحص المطلوب | المصدر | إجراء الرواف.

# 9. البرنامج الزمني والمواعيد والالتزامات
جدول: الالتزام | التاريخ/المدة | المصدر | خطورة عدم الالتزام | إجراء الرواف.

# 10. جدول الكميات والأرقام المؤثرة
جدول: رقم البند | الوصف | الكمية | الوحدة | الدلالة الفنية | المصدر. ثم قسم مستقل [بنود كمية صفر تحتاج تحقق] إن وجدت.

# 11. الشروط المالية/التعاقدية المؤثرة على العرض الفني
جدول: الشرط | الرقم/النسبة/المدة | المصدر | أثره على العرض الفني/المخاطر | إجراء الرواف.

# 12. سجل التعارضات والفجوات ونقاط الاستفسار
جدول: رقم | النوع | الوصف | سبب الخطورة | الملفات ذات العلاقة | نص الاستفسار المقترح.

# 13. خطة عمل إعداد العرض الفني لشركة الرواف
جدول: الأولوية | الإجراء | المسؤول المقترح | المخرج المطلوب | الموعد الداخلي المقترح.
"""

TENDER_REPAIR_PROMPT = """أنت مراجع أول لعروض فنية. التقرير السابق فشل لأنه عام أو فقير.

أعد إنتاج تقرير تحليل مناقصة كامل من سجلات الملفات المتاحة. المطلوب تعميق مصفوفة متطلبات الجهة المالكة تحديداً.

قواعد الإصلاح:
- لا تبدأ بملخص طويل؛ اجعل التقرير عملياً.
- استخرج 45 متطلباً على الأقل إذا كانت السجلات تسمح.
- افصل المتطلبات الصغيرة: الضمان، النسخ، النماذج، التوقيعات، التقارير، الخطط، التأهيل، التصاريح، الاختبارات، الاعتمادات، الكميات، المحتوى المحلي.
- لكل متطلب: المصدر، الأثر، إجراء شركة الرواف.
- لا تستخدم عبارات عامة إلا إذا كانت النصوص نفسها عامة؛ وفي هذه الحالة اكتب [النص المتاح عام ويحتاج مراجعة أصل الملف].
- لا تخترع أي رقم أو مدة أو جهة.

أعد التقرير بنفس هيكل TENDER_SYNTHESIS_PROMPT وبنبرة عربية مؤسسية.
"""

def _count_owner_requirement_rows(report: str) -> int:
    """Count likely rows inside the owner-requirements matrix."""
    if not report:
        return 0
    m = re.search(r"#\s*4\.?\s*.*?مصفوفة متطلبات الجهة المالكة.*?(?=\n#\s*5\.|\Z)", report, re.DOTALL)
    ...
    for line in sec.splitlines():
        ...
        if not s.startswith("|"):
            continue
        if "---" in s or "المتطلب" in s or "رقم" in s:
            continue
...
def _is_weak_tender_report(report: str) -> bool:
    """Quality gate: detect generic/weak tender reports and trigger deep repair."""
    if not report or report.startswith("[AI Error"):
        return False
    wc = word_count(report)
    req_rows = _count_owner_requirement_rows(report)
    required_markers = [
        "مصفوفة متطلبات الجهة المالكة", "بطاقة حقائق المشروع", "سجل وثائق المناقصة",
        "المتطلبات الفنية", "المنهجيات والخطط", "جدول الكميات", "الفجوات", "توصيات إعداد العرض الفني"
    ]
    missing_markers = sum(1 for m in required_markers if m not in report)
    generic_phrases = [
        "الالتزام بالمعايير", "تقديم جميع الوثائق", "توفير الكوادر", "استخدام المعدات الحديثة",
        "القوانين واللوائح المحلية والدولية", "وفقًا للمواصفات المحددة", "المعايير الدولية والمحلية",
        "معدات ثقيلة مثل الحفارات والرافعات",
    ]
    generic_hits = sum(report.count(p) for p in generic_phrases)
    return wc < 3800 or req_rows < 40 or missing_markers >= 2 or generic_hits >= 6


def _build_executive_evidence_pack(tender_texts: dict, model: str) -> str:
    """Pull distributed keyword snippets from all files to prevent generic summaries."""
    keyword_re = re.compile(
        ...
        r"محتوى محلي|الحد الأدنى|نسبة|تقرير|دوري|نهائي|مورد|باطن|عمالة|معدات|كود|مواصفة|SASO|AASHTO|ACI|ASTM|ISO|"
    ...
    for name, txt in tender_texts.items():
        if not txt:
            continue
        clean = re.sub(r"\n{3,}", "\n\n", txt)
        matches = list(keyword_re.finditer(clean))
        selected = []
        used = []
        step = max(1, len(matches) // 35) if matches else 1
        for m in matches[::step][:35]:
            a = max(0, m.start() - 450)
            b = min(len(clean), m.end() + 900)
        ...
        if not selected:
            selected = [clean[:3500]]
        part = f"\n\n{'='*70}\nحزمة أدلة مركزة من الملف: {name}\n{'='*70}\n" + "\n\n--- مقتطف ---\n".join(selected)
        chunks.append(truncate_to_token_budget(part, 9000, model=model))
    return truncate_to_token_budget("\n\n".join(chunks), 50000, model=model)


def analyze_tender_in_batches(client, tender_texts: dict) -> str:
    """
    Production-grade tender analysis engine.
    The user sees one final report, but internally the engine extracts evidence per file first.
    """
    if not tender_texts:
    ...
    def _single_pass() -> str:
        tender_context = build_context_bundle(
            ...
            per_file_tokens=max(3500, int(MAX_SINGLE_PASS_TENDER_TOKENS / max(len(tender_texts), 1))),
            model=model,
        )
        evidence_pack = _build_executive_evidence_pack(tender_texts, model=model)
        user_msg = (
            "أنت تحلل وثائق مناقصة لصالح مقدم العرض: شركة الرواف.\n"
            "المطلوب تقرير واحد غني ومحدد، وليس ملخصاً. ركّز على كل ما تطلبه الجهة المالكة من مقدم العرض.\n"
            "استخدم النصوص العامة وحزمة الأدلة المركزة أدناه. عند التعارض لا تحسم، بل صنّف [تعارض].\n\n"
            "## النصوص المحدودة للتحليل\n"
            f"{tender_context}\n\n"
            "## حزمة الأدلة المركزة ذات الصلة بالمتطلبات\n"
            f"{evidence_pack}"
        )
        return call_ai(client, TENDER_ANALYSIS_PROMPT, user_msg, max_tokens=TENDER_REPORT_MAX_TOKENS, temperature=0.02)

    def _staged_pass() -> str:
        per_file_summaries = []
        total = len(tender_texts)
        for i, (name, txt) in enumerate(tender_texts.items(), start=1):
            file_context = build_compact_context_from_file(name, txt, max_chars=62000, max_tokens=MAX_INPUT_TOKENS_PER_FILE)
            evidence = _build_executive_evidence_pack({name: txt}, model=model)
            file_prompt = (
                f"حلل الملف رقم {i} من {total} لصالح مقدم العرض: شركة الرواف.\n"
                f"اسم الملف: {name}\n"
                f"عدد الكلمات التقريبي: {word_count(txt):,}\n\n"
                "لا تلخص. استخرج سجل متطلبات تفصيلي من هذا الملف فقط. "
                "أي متطلب غامض اكتبه كنقطة تحقق. أي رقم/مدة/نسبة/ضمان/غرامة/كمية/موعد يجب نقله كما ورد. "
                "استخرج متطلبات التقديم والوثائق والتأهيل والفني والجودة والسلامة والبيئة والبرنامج الزمني والموارد والمحتوى المحلي.\n\n"
                f"## النص المحدود الآمن\n{file_context}\n\n"
                f"## مقتطفات أدلة مركزة من الملف\n{evidence}"
            )
            summary = call_ai(client, SINGLE_FILE_TENDER_PROMPT, file_prompt, max_tokens=8500, temperature=0.02)
            if summary.startswith("[AI Error"):
                per_file_summaries.append(f"# {name}\nتعذر تحليل هذا الملف: {summary}")
            else:
                per_file_summaries.append(f"# {name}\n{truncate_to_token_budget(summary, 7000, model=model)}")
            time.sleep(TENDER_ANALYSIS_SLEEP_SECONDS)

        merged_summaries = "\n\n".join(per_file_summaries)
        final_input = (
            "فيما يلي سجلات متطلبات مستخرجة من كل ملف على حدة. أصدر تقريراً نهائياً واحداً لصالح شركة الرواف.\n"
            "هذا التقرير سيُستخدم لإعداد العرض الفني، لذلك يجب أن يكون كثيفاً ومحدداً.\n"
            "ممنوع الاكتفاء بعبارات عامة. المطلوب مصفوفة متطلبات لا تقل عن 45 متطلباً إذا سمحت الوثائق، مع مصدر وإجراء للرواف.\n"
            "إذا لم تسمح الوثائق بعدد كبير، اذكر أن النص المتاح لا يكفي ولا تخترع.\n\n"
            + truncate_to_token_budget(merged_summaries, MAX_SYNTHESIS_INPUT_TOKENS, model=model)
        )
        result = call_ai(client, TENDER_SYNTHESIS_PROMPT, final_input, max_tokens=TENDER_REPORT_MAX_TOKENS, temperature=0.03)
        if _is_weak_tender_report(result):
            repair_input = (
                "التقرير السابق لا يزال ضعيفاً أو عاماً. أعد بناءه بالكامل من سجلات الملفات التالية.\n"
                "ركّز على جدول مصفوفة متطلبات الجهة المالكة من مقدم العرض واجعله عميقاً ومفصلاً.\n"
                "لا تكرر توصيات عامة. أخرج متطلبات محددة قابلة للتنفيذ في العرض الفني لشركة الرواف.\n\n"
                + truncate_to_token_budget(merged_summaries, MAX_SYNTHESIS_INPUT_TOKENS, model=model)
            )
            repaired = call_ai(client, TENDER_REPAIR_PROMPT, repair_input, max_tokens=TENDER_REPORT_MAX_TOKENS, temperature=0.02)
            if not repaired.startswith("[AI Error"):
                return repaired
        return result
...
السياق: مقدم العرض هو شركة الرواف. المطلوب تحليل وثائق الجهة المالكة لاستخراج كل ما يجب على شركة الرواف الالتزام به في العرض الفني والتقديم.
...
2. لا تخلط بين مشاريع أو جهات أو مواقع. إذا ظهرت جهات أو مواقع متعارضة، ضعها تحت [تعارض] ولا تحسمها.
3. لكل معلومة مهمة اذكر مصدرها باسم الملف كما يظهر في السياق. إذا ظهر بند أو عنوان داخل النص فاذكره كذلك.
4. استخرج كل متطلب تطلبه الجهة المالكة من مقدم العرض، وليس فقط وصف المشروع.
5. استخرج جميع الأرقام والنسب والمدد والكميات والمواعيد والغرامات والضمانات وشروط التقديم كما وردت.
6. افصل بين: متطلبات فنية، متطلبات تقديم، متطلبات منهجيات، متطلبات موارد، متطلبات جودة، متطلبات سلامة، متطلبات برنامج زمني، ومتطلبات مستندات.
7. إذا لم تظهر المعلومة في الوثائق، اكتب: [غير متوفر في الوثائق المرفوعة].
8. لا تستخدم لغة عامة مثل "حسب المواصفات" فقط؛ اذكر نص/مضمون المتطلب المحدد إن ظهر.
9. إذا وجدت بنداً بكمية صفر في جدول الكميات، اذكره في قسم مستقل بعنوان [بنود كمية صفر تحتاج تحقق] ولا تدرجه ضمن النطاق التنفيذي المعتمد.
10. التقرير الضعيف المكوّن من عبارات عامة مرفوض. المطلوب سجل متطلبات واضح وقابل للتحويل إلى خطة عرض فني.

هيكل التقرير المطلوب:

# 📋 الملخص التنفيذي للمنافسة
- طبيعة المشروع كما تظهر في الوثائق فقط.
- درجة وضوح الوثائق.
- أهم متطلبات الجهة المالكة من مقدم العرض، بصياغة محددة لا عامة.
- أهم مخاطر أو فجوات أو تعارضات تؤثر على قرار التقديم.

# 1. بطاقة حقائق المشروع
جدول يحتوي على: اسم المشروع، رقم المنافسة، الجهة المالكة، الجهة المشرفة، موقع الأعمال، مدة المشروع، نوع العقد/الاتفاقية، موعد التقديم، نطاق العمل، مصدر كل معلومة، حالة الثقة: مؤكد/يحتاج تحقق/تعارض/غير متوفر.

# 2. سجل وثائق المناقصة المحللة
جدول يحتوي على: اسم الملف، نوع الوثيقة، أهم المعلومات التي وفرها، أهم المتطلبات المستخرجة منه، ملاحظات القراءة أو النقص.

# 3. نطاق العمل المستخلص
اعرض نطاق الأعمال والحزم التنفيذية كما تظهر في الوثائق، مع عدم إضافة أعمال غير مذكورة. لكل حزمة اذكر المصدر والقيود.

# 4. مصفوفة متطلبات الجهة المالكة من مقدم العرض
جدول تفصيلي لا يقل عن 45 متطلباً إذا سمحت الوثائق، بالأعمدة التالية:
- رقم
- المتطلب المطلوب من مقدم العرض
- التصنيف: فني / إداري / تقديم / جودة / سلامة / بيئة / زمني / موارد / وثائق / مالي-تعاقدي
...
# 5. المتطلبات الفنية والكودية والاختبارات
استخرج المواصفات الفنية، الأكواد، المعايير، المواد، الاختبارات، حدود القبول، متطلبات الاعتماد، العينات، ومتطلبات التنفيذ الواضحة.

# 6. المنهجيات والخطط المطلوبة في العرض الفني
اذكر كل منهجية أو خطة أو بيان طريقة تنفيذ تطلبه الوثائق صراحة أو يلزم لإثبات الامتثال الفني، مع مصدر كل مطلب ومحتوى مقترح للعرض.

# 7. الموارد والكوادر والمعدات المطلوبة
استخرج أي متطلبات للهيكل التنظيمي، الخبرات، المؤهلات، العمالة، المعدات، الموردين، المقاولين الباطن، أو المحتوى المحلي. لا تخترع أعداداً غير مذكورة.

# 8. متطلبات الجودة والسلامة والبيئة والتصاريح
استخرج متطلبات ضبط الجودة، الفحوصات، السلامة، الصحة المهنية، البيئة، التصاريح، التنسيق، وخطط الموقع.

# 9. البرنامج الزمني والمواعيد والالتزامات الزمنية
استخرج مدة المشروع، تواريخ التقديم، الاستفسارات، الزيارات، الإنجاز، الضمانات، القيود الزمنية، وفترات الصلاحية.

# 10. جدول الكميات والأرقام المؤثرة
استخرج البنود والكميات والأرقام الحرجة التي تؤثر على النطاق الفني، مع تمييز أي بنود كمية صفر إن وجدت.

# 11. الشروط التعاقدية المؤثرة على العرض الفني
استخرج شروط الدفع، الغرامات، الضمانات، التأمينات، الفيديك أو الشروط الخاصة، وأثرها على التجهيز الفني.

# 12. الفجوات والتعارضات ونقاط الاستفسار
جدول بالأعمدة: رقم، الفجوة/التعارض/السؤال، سبب الأهمية، المصدر، الإجراء المقترح.

# 13. توصيات إعداد العرض الفني لشركة الرواف
